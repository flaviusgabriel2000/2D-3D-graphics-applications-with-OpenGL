...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_Z) && perspective) {
        fov -= deltaTime;
        projectionMatrix = glm::perspective(fov, aspect, zNearP, zFarP);
    }
    if (window->KeyHold(GLFW_KEY_X) && perspective) {
        fov += deltaTime;
        projectionMatrix = glm::perspective(fov, aspect, zNearP, zFarP);
    }


    if (window->KeyHold(GLFW_KEY_C) && ortho) {
        orthoLeft += deltaTime;
        orthoRight -= deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, zNearO, zFarO);
    }
    if (window->KeyHold(GLFW_KEY_V) && ortho) {
        orthoLeft -= deltaTime;
        orthoRight += deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, zNearO, zFarO);
    }
    if (window->KeyHold(GLFW_KEY_B) && ortho) {
        orthoBottom += deltaTime;
        orthoTop -= deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, zNearO, zFarO);
    }
    if (window->KeyHold(GLFW_KEY_N) && ortho) {
        orthoBottom -= deltaTime;
        orthoTop += deltaTime;
        projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, zNearO, zFarO);
    }
