...
void main()
{
    ...
    float ambient_light = material_kd * 0.7f;
    ...
    for (i = 0; i < 15; i++) {
        ...
        float ambient_light = material_kd * 0.05f;
        float diffuse_light = material_kd * max(dot(N, L), 0);
        float specular_light = 0;
    ...
    vec4 colorTex = texture2D(textureSampler, texcoord);

    for (i = 0; i < 20; i++) {
        spotLight += spotLights[i];
        if (i < 15) {
           pointLight += vec4(pointLights[i], 1.0f) * colorTex; 
        }
    }
    ...
    if (color.x == 1 && color.y == 1 && color.z == 1) {
        ...
        out_color = (directionalLight + spotLight) * colorTex + pointLight;
    } else {
