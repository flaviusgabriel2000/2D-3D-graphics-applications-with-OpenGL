...
void main()
{
    ...
    float ambient_light = material_kd * 0.15f;
    ...
    for (i = 0; i < 20; i++) {
        if (spot_light_pos[i].x == 0 && spot_light_pos[i].y == 0 && spot_light_pos[i].z == 0) {
            spotLights[i] = 0;
            continue;
        }
    ...
    for (i = 0; i < 20; i++) {
        ...
        if (i < 15) {
            pointLight += vec4(pointLights[i], 1.0f) * color;
        }    
    }
    out_color = (spotLight + directionalLight) * color + pointLight;
}
