...
void Lab2::Init()
{
    ...
    {
        //VertexFormat(glm::vec3(-1, -1,  1), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(3, 3,  0), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(2, 0,  0), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(3.5f, 0,  2), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(4.5f, 0,  0), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        // TODO(student): Complete the vertices data for the cube mesh
    ...
    for (int i = 0; i < 1000; i++) {
        circleVertices.push_back(VertexFormat(glm::vec3(cos(2 * 3.14159 * i / 1000.0), sin(2 * 3.14159 * i / 1000.0), 0), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)));
    }
    ...
    CreateMesh("circle", circleVertices, circleIndices);
...
void Lab2::CreateMesh(const char* name, const std::vector<VertexFormat>& vertices, const std::vector<unsigned int>& indices)
{
    ...
    if (!strcmp(name, "circle")) {
        meshes[name]->SetDrawMode(GL_LINE_STRIP);
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.75f));

    // TODO(student): Draw the mesh that was created with `CreateMesh()`
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(2, 0.5f, -1.5f), glm::vec3(0.75f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(3, 0.5f, 1.5f), glm::vec3(0.5f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, 1, 1), glm::vec3(0.5f));

    RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(0, 2, 1), glm::vec3(0.5f));
